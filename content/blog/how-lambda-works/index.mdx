...
- it's the most well-known representative of "serverless" architecture

Which leads us to our first question... what the heck is serverless, anyway? And how does Lambda work?
...
**Instead, you write and deploy *individual functions.***
...
Well, your functions are run in response to *events.*
...
A user uploads a new image to S3 (which is AWS' simple storage solutions)).
...
Lambda functins are *versioned* and *aliased*.
...
You can also do partial allocation, like routing 60% of traffic to **prod** to v1, 40% to v2... and then slowly ramp up v2. Bit safer that way.
...
When you deploy a typical back-end application, you deploy *instances* to *servers*. Depending on your setup, you then have to manage those instances and servers.

How much management? Depends. But at the very least, you hae to keep your instances running... all the time.
...
**More importantly**, I just rebooted my email newsletter. Check it out. Get articles like this + other cool stuff, straight into your inbox, once a week.
