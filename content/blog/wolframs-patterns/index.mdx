...
The holy grail of physics is the so-called Theory of Everything, a single framework that explains all physical aspects of the universe. Right now, we have the theory of general relativity, which works very well for big things, and we have the theory of quantum mechanics, which works well for very small things, but we have no single theory to explain them both.

Ideally, the Theory of Everything would rest on a single set of equations that explain... well, everything. Everything from the beginning of the universe, when all matter was squished together before the Big Bang, all the way to our present day, expanding universe.
...
Don't worry too much about explaining the rule, but watch the pattern:

<WolframPattern defaultRule={30} />

The left side of this pattern looks pretty predictable. But the rest is not. It follows no predictable pattern at all. In fact, if you execute the pattern for 500 rows...
...
... you can see it forms a lot of noise on the right side. It's entirely unpredictable, or random.
...
When we're defining a rule for our grid pattern, we can then represent with the above 8. For example, if the rule is "only fill the cell if all three cells above it are filled", we can show it in the following table:
...
The widget below lets you build your own patterns from this binary notation. 

<BinaryWolframPattern />

Try making Rule 110, which is like so:

TABLE

Even with these limited inputs (filled/unfilled) and limited space (our tiny grid), we were able to generate complex, unpredictable behaviour. It's not unreasonable to think that our universe might work the same way: an entire reality generated from a few mathematical rules.
